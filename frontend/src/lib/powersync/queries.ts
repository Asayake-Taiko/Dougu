export const Queries = {
    User: {
        ...
        insert: 'INSERT INTO users (id, email, full_name, profile, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)',
    ...
    Organization: {
        ...
        insert: 'INSERT INTO organizations (id, name, access_code, manager_id, created_at) VALUES (?, ?, ?, ?, ?)',
    ...
    Membership: {
        ...
        `,
        getByOrgAndUser: 'SELECT * FROM org_memberships WHERE organization_id = ? AND user_id = ?',
        getDetailsByOrgAndUser: `
            SELECT m.*, u.full_name, u.profile as user_profile
            FROM org_memberships m 
            LEFT JOIN users u ON m.user_id = u.id 
            WHERE m.organization_id = ? AND m.user_id = ?`,
        insert: 'INSERT INTO org_memberships (id, organization_id, user_id, type) VALUES (?, ?, ?, ?)',
    },
    Container: {
        getAllByOrg: 'SELECT * FROM containers WHERE organization_id = ?',
        updateAssignment: 'UPDATE containers SET assigned_to = ?, last_updated_date = ? WHERE id = ?',
    ...
    Equipment: {
        ...
        updateAssignment: 'UPDATE equipment SET assigned_to = ?, container_id = ?, last_updated_date = ? WHERE id = ?',
        delete: 'DELETE FROM equipment WHERE id = ?',

        // Batch update helper for container re-assignment (if needed directly)
        updateAssignmentByContainer: 'UPDATE equipment SET assigned_to = ?, last_updated_date = ? WHERE container_id = ?',
    }
};
