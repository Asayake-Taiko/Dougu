...
export default function EquipmentTable({
    ...
    const tableData = useMemo(() => {
        ...
        ownerships.forEach((ownership: OrgOwnership) => {
            ownership.items.forEach((item: Item) => {
                data.push(Object.assign(item, { ownerName: ownership.membership.name }));
            });
        });
        return data;
    }, [ownerships]);
    ...
    const filteredData = useMemo(() => {
        ...
        return tableData.filter((item: Item & { ownerName: string }) =>
            item.name.toLowerCase().includes(filter) ||
            item.ownerName.toLowerCase().includes(filter) ||
            (item.type === "container" && (item as Container).equipment.some(e => e.name.toLowerCase().includes(filter)))
        );
