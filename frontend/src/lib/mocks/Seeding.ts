...
export async function seedDatabase(db: PowerSyncDatabase) {
    await db.writeTransaction(async (tx) => {
        ...
        const tables = ['users', 'organizations', 'org_memberships', 'containers', 'equipment'];
        ...
        for (const user of MOCK_USERS) {
            await tx.execute(
                ...
                [user.id, user.email, user.full_name, user.profile, user.created_at, user.updated_at]
            );
        }
        ...
        for (const org of MOCK_ORGS) {
            await tx.execute(
                ...
                [org.id, org.name, org.access_code, org.manager_id, org.image, org.created_at]
            );
        }
        ...
        for (const membership of MOCK_MEMBERSHIPS) {
            await tx.execute(
                ...
                [membership.id, membership.organization_id, membership.type, membership.user_id, membership.group_name, membership.profile, membership.details]
            );
        }
        ...
        for (const container of MOCK_CONTAINERS) {
            await tx.execute(
                ...
                [container.id, container.name, container.organization_id, container.assigned_to, container.color, container.group_name, container.details, container.last_updated_date]
            );
        }
        ...
        for (const gear of MOCK_EQUIPMENT) {
            await tx.execute(
                ...
                [gear.id, gear.name, gear.organization_id, gear.assigned_to, gear.container_id, gear.image, gear.color, gear.group_name, gear.details, gear.last_updated_date]
            );
        }
    });
}
