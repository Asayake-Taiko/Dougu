...
describe("PowerSync Sync Rules", () => {
  ...
  it("User has data of (equipment/container/membership/profile) of an organization they are a part of", async () => {
    ...
    try {
      ...
      const equipments = await db.getAll(
      ...
      const profiles = await db.getAll("SELECT * FROM profiles WHERE id = ?", [
        user.user.id,
      ]);
  ...
  it("User does not have data of (equipment/container/membership/profile) of an organization they are not a part of", async () => {
    ...
    const equipmentB = await createEquipment(
      userB.token,
      orgB.id,
      "Equipment B",
    );
    ...
    try {
      ...
      const orgs = await db.getAll("SELECT * FROM organizations WHERE id = ?", [
        orgB.id,
      ]);
      const containers = await db.getAll(
        "SELECT * FROM containers WHERE id = ?",
  ...
  it("User can join an org and gain new access", async () => {
    ...
    try {
      // Initially no access
      const resultsInitial = await db.getAll(
        "SELECT * FROM organizations WHERE id = ?",
        [orgA.id],
      );
  ...
  it("User has access to data of multiple organizations they are a part of", async () => {
    ...
    db = await createPowerSyncClient(
      user.token,
      getDbPath("test_multiple_orgs"),
    );

    try {
      await waitFor(async () => {
        const results1 = await db.getAll(
          "SELECT * FROM organizations WHERE id = ?",
          [org1.id],
        );
