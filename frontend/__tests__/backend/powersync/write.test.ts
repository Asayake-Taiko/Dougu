...
describe("PowerSync Write Operations", () => {
  ...
  it("PATCH: Reassigning equipment using PowerSync updates data in Supabase", async () => {
    ...
    db = await createPowerSyncClient(
      user.token,
      getDbPath("test_patch_equipment"),
    );
  ...
  it("PATCH: Reassigning container using PowerSync updates data in Supabase", async () => {
    ...
    db = await createPowerSyncClient(
      user.token,
      getDbPath("test_patch_container"),
    );
  ...
  it("PATCH: Batch update equipment in container using PowerSync updates data in Supabase", async () => {
    ...
    try {
      ...
      await waitFor(async () => {
        const results = await db.getAll(
          "SELECT * FROM equipment WHERE container_id = ?",
          [container.id],
        );
      ...
      expect(supabaseData?.every((e) => e.assigned_to === membershipId)).toBe(
        true,
      );
    } finally {
  ...
  it("DELETE: Deleting equipment via PowerSync removes from Supabase", async () => {
    ...
    db = await createPowerSyncClient(
      user.token,
      getDbPath("test_delete_equipment"),
    );
  ...
  it("PATCH: Multiple sequential equipment updates sync correctly", async () => {
    ...
    try {
      ...
      const membershipIdB = await getMembershipId(
        user.token,
        org.id,
        userB.user.id,
      );
      const membershipIdC = await getMembershipId(
        user.token,
        org.id,
        userC.user.id,
      );
