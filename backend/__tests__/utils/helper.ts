...
export async function waitFor(predicate: () => Promise<boolean>, timeout = 15000, interval = 500) {
...
export async function createContainer(accessToken: string, orgId: string, name: string) {
  const userClient = createClient(accessToken);
...
export async function createEquipment(accessToken: string, orgId: string, name: string) {
  const userClient = createClient(accessToken);
...
export async function joinOrg(userAccessToken: string, orgId: string, userId: string) {
...
export async function getMembershipId(accessToken: string, orgId: string, userId: string) {
  const userClient = createClient(accessToken);
...
export async function removeMembership(adminAccessToken: string, orgId: string, userId: string) {
